...
final class QuestionBankAsyncFlowTests: XCTestCase {
    ...
    @MainActor
    func testRestoredPolicyOneContextReplacesBlockedBankWithoutRelabelingHistory() async throws {
        let suiteName = "QuestionBankPolicyMigrationTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        // Exported by the real policy-1 Store at ed38bdd using a mock bank.
        // Its context was generated by that app, not reproduced by this test.
        let fixtureURL = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
            .appendingPathComponent("Fixtures/policy-one-bank-snapshot.json")
        var snapshot = try QuestionContentJSONDecoder.decode(AppSnapshot.self, from: Data(contentsOf: fixtureURL))
        let historicalQuestion = try XCTUnwrap(snapshot.questions.first)
        let historicalAttempt = try XCTUnwrap(snapshot.attempts.first)
        var oldIntent = try XCTUnwrap(snapshot.questionBankSyncIntents?.first)
        XCTAssertEqual(oldIntent.contextRevision, "3a2db6485fc9ea3f")
        XCTAssertEqual(historicalQuestion.verificationPolicyRevision, 1)
        oldIntent.generationBlockedReason = "provider_failure_limit"
        oldIntent.emptyFillCycleRetryCount = 1
        snapshot.questionBankSyncIntents = [oldIntent]
        ...
        XCTAssertEqual(client.ensureRequests.first?.requiresVerifiedQuestions, true)
        XCTAssertNil(store.nextQuestion())
        XCTAssertEqual(store.questions, [historicalQuestion], "Policy migration must preserve stored question content and its old stamp.")
        XCTAssertEqual(store.attempts, [historicalAttempt], "Policy migration must not rewrite or rescore history.")
        let saved = try XCTUnwrap(defaults.data(forKey: AppSnapshotPersistence.primaryDefaultsKey))
        let restored = try QuestionContentJSONDecoder.decode(AppSnapshotEnvelope.self, from: saved).snapshot
        XCTAssertEqual(restored.questions, [historicalQuestion])
        XCTAssertEqual(restored.attempts, [historicalAttempt])
        XCTAssertEqual(restored.questionBankSyncIntents?.first, intent)
    }
