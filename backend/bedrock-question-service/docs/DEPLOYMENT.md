...
- `SkillMapModelArn` and `SkillMapInvokeResourceArns`: optional API-only skill-map model/profile and explicit invocation resources. Leave both empty to preserve inheritance from the worker. To keep skill-map planning on Kimi while the worker uses Sonnet, provide the Kimi ARN and its complete resource allowlist through `SKILL_MAP_MODEL_ARN` and `SKILL_MAP_INVOKE_RESOURCE_ARNS`. This changes neither the API question model nor the worker model, transport, or permissions. Both deployment entry points reject partial/invalid overrides; global native API mode also checks the effective skill-map model against the native allowlist.
