...
| `SKILL_MAP_MODEL_ID` | falls back to `BEDROCK_MODEL_ID` locally; `QuestionBankWorkerModelArn` in SAM | Model used only by synchronous skill-map planning. SAM inherits the worker model by default. Optional `SkillMapModelArn` plus `SkillMapInvokeResourceArns` pin only API skill-map planning independently, while synchronous questions retain `BEDROCK_MODEL_ID` and the worker retains its selected model. |
...
| `QUESTION_AUTHOR_MODE` | `prose` | Opt-in `mixed_quantitative` uses exact typed numerical specs when the entire objective fits the bounded compiler, with ordinary prose for other subjects. Requires native transport. It can use reviewer-written feedback or the opt-in immutable-main audit. Compiled rows preserve all five derived learner fields and receive server policy6 after existing gates; ordinary rows receive policy4 with reviewer-written feedback or policy7 with exact authored main and empty choice feedback. SAM/workflow worker override defaults to inherit. See the [contract, limits and unqualified rollout status](../../docs/QUANTITATIVE_TASK_COMPILER.md). |
| `QUESTION_FEEDBACK_CONTRACT` | `reviewer_written` | Server-only opt-in `authored_solution` preserves the author's complete main explanation and audits it without allowing the reviewer to write replacement teaching. Native prose requires count-bound complete-pair solving and an immutable main audit (policy 7); privately revalidated mixed compiler rows use local mathematical proof and the same final audit (policy 8). Reviewer-written compiled mode retains its solver and policy 6; legacy mode retains its complete-choice audit (policy 3). Model-supplied choice feedback is rejected; the mixed route retains only exact feedback revalidated against a private compiler sidecar. See the [contract and qualification status](../../docs/QUESTION_AUTHORED_SOLUTION_CONTRACT.md). Worker-only SAM parameter `QuestionBankWorkerFeedbackContract` and workflow variable `QUESTION_BANK_WORKER_FEEDBACK_CONTRACT` default to `reviewer_written`; the API default is unchanged. |
| `BEDROCK_FALLBACK_MODEL_ID` | empty | Optional secondary model ARN; enable only after it passes the same eval suite. |
| `BEDROCK_STRUCTURED_OUTPUT_MODE` | `legacy` | Effective per-function transport mode: `legacy` preserves prompt-requested JSON; `native` sends a versioned, stage-specific `outputConfig.textFormat` schema. Native mode fails closed for missing contracts, models outside the capability allowlist, and SDK/service schema incompatibility; it never retries without a schema. Qualify every model/stage used by the function being enabled, including its configured fallback. SAM uses `BedrockStructuredOutputMode` for the API and a separate worker override. The documented synchronous Nova Lite configuration is not native-compatible. |
...
preserving the frozen v1/v2 schema bytes. This guarantees the choice count and
key membership, not distinct meanings, factual correctness, or valid teaching;
independent solving and final review still control admission. Legacy generation
keeps v1. The [bounded author comparison](../../docs/evidence/native-author-order-20260922/REPORT.md)
records the observed improvement and remaining ambiguity; the earlier
[fixed-slot probe](../../docs/evidence/native-author-release-20260922/REPORT.md)
retains its failed explanation bounds and incorrect-answer evidence.
...
The SAM parameter `QuestionBankWorkerStructuredOutputMode` and deploy workflow
variable `QUESTION_BANK_WORKER_STRUCTURED_OUTPUT_MODE` accept `inherit` (default),
`legacy`, or `native`. They resolve the worker's existing
`BEDROCK_STRUCTURED_OUTPUT_MODE` environment variable without changing the API's
mode or any model selection. A qualified worker can use explicit `native` while
the Nova Lite API remains `legacy`. The deployment scripts validate the effective
mode and every configured author, verifier and fallback against the runtime
capability allowlist before SAM deployment. That check establishes configuration
compatibility, not model quality. Roll the worker back with explicit `legacy`;
`inherit` follows the global mode, including a global `native` setting. Clients
requiring policy revision 4 need the complete native slot/pair path; coordinate
any such client promotion with worker rollout. Never relabel old inventory to
satisfy a new minimum policy. Checked-in defaults and the current client minimum
remain unchanged while semantic qualification continues.
...
revision 4. The legacy complete-choice path stays at revision 2; optional
`authored_solution` uses revision 7 with native count-bound pair solving and immutable main auditing, or revision 3 with legacy transport. Historical stem-only evals retain revision 1. The default remains 4; revision minimums are freshness thresholds, not cumulative capabilities.

Final feedback must cover the exact choices and meet the unchanged client bounds.
Duplicate JSON properties are rejected rather than silently selecting a value.
The separate opt-in `QUESTION_AUTHOR_MODE=constructed_quantitative` sends task-only quantitative rows without choices. Native transport and `QUESTION_FEEDBACK_CONTRACT=authored_solution` are mandatory before any provider reservation. The new `question_author_constructed_v1` grammar retains existing ordered prose rows; numeric tasks use exact expressions or explicit integer-interval scalar conditions. Code validates/translates the graph, constructs four distinct choices through the [bounded constructor](../../docs/QUANTITATIVE_CHOICE_CONSTRUCTION.md), recompiles all five learner fields and supplies private provenance. Unsupported domains, provider choices, malformed graphs and inadequate distractor pools are rejected without repair. Prose still passes the blind solver and immutable main audit (policy 7); compiled rows pass local proof and the same final audit (policy 8). Historical mixed/prose contracts and all defaults are unchanged. This candidate is not provider- or worker-qualified.

SAM exposes the new mode only through `QuestionBankWorkerAuthorMode=constructed_quantitative` (`QUESTION_BANK_WORKER_AUTHOR_MODE` in the workflow), together with worker native transport and authored-solution feedback. The existing deploy-script forwarding needs no new variable; validation rejects incompatible combinations before SAM runs. The global API author options/default remain unchanged. No model, token, timeout, call budget, policy revision, or client floor changes are part of this mode.

Native mixed authored-solution compiler rows skip the redundant correctness/pair solver only after exact private revalidation; prose retains that solver and all survivors still receive final audit. An all-compiled pass uses two provider calls, a mixed pass three, within the unchanged six-call budget and conservative three-call pre-author minimum. These checks enforce
structure and the models' declarations; confident semantic mistakes can still
pass. The [structural milestone and live failures](../../docs/evidence/question-reliability-release-20260922/STRUCTURAL_MILESTONE.md)
record 1,124 passing backend tests, a failed strict pair-label qualification and
29 admitted fresh questions with remaining content errors. No deployment or
claim of deterministic factual correctness follows from those results.
