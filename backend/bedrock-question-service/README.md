...
| `BEDROCK_VERIFICATION_STRUCTURED_OUTPUT_MODE` | `inherit` | Optional `legacy` or `native` override for the complete-choice solver and either reviewer. Authoring and skill-map calls keep the global mode. This permits a legacy Nova Lite author with native Sonnet verification without weakening the parser or adding calls. SAM, deploy script and workflow carry the override; `inherit` preserves existing behavior. Configure and qualify the actual verifier model before changing it. |
