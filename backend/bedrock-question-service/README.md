...
| `QUESTION_AUTHOR_MODE` | `prose` | Opt-in `mixed_quantitative` uses exact typed numerical specs when the entire objective fits the bounded compiler, with ordinary prose for other subjects. Requires native transport and reviewer-written mode. Compiled rows preserve all five derived learner fields and receive server policy6 after existing gates; ordinary rows retain their existing revision. SAM/workflow worker override defaults to inherit. See the [contract, limits and unqualified rollout status](../../docs/QUANTITATIVE_TASK_COMPILER.md). |
| `QUESTION_FEEDBACK_CONTRACT` | `reviewer_written` | Server-only opt-in `authored_solution` preserves the author's complete main explanation and audits it without allowing the reviewer to write replacement teaching. Native mode requires count-bound complete-pair solving and an immutable main audit (policy 7); legacy mode retains its complete-choice audit (policy 3). Nonempty incoming choice feedback is rejected. See the [contract and qualification status](../../docs/QUESTION_AUTHORED_SOLUTION_CONTRACT.md). Not wired to a new SAM deployment parameter or enabled by default. |
