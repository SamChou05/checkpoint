...
| `BEDROCK_VERIFICATION_STRUCTURED_OUTPUT_MODE` | SAM/deploy: `native`; unset process environment: `inherit` | Independent transport for the complete-choice solver and either reviewer. Authoring and skill-map calls keep the global mode (`legacy` by default), so Nova Lite remains compatible while Sonnet verification uses native JSON. `legacy` and `inherit` remain explicit overrides. Native mode requires a qualified supported verifier and the packaged SDK. It constrains response shape, not semantic correctness. Existing deployments change only when redeployed with these settings. |
