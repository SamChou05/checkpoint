...
| `BEDROCK_VERIFICATION_MODEL_ID` | `us.anthropic.claude-sonnet-4-6` locally; explicit `BedrockVerificationModelArn` in SAM | Separate answer-blind reviewer. Every new bank question requires agreement on correctness; logical inference proofs, choice bounds, and review text are additionally checked in code. |
...
| `QUESTION_BANK_MAX_RECEIVE_COUNT` | `5` | Shared SQS redrive and actual per-job Bedrock Converse-call threshold. |
...
Use `python evals/checkpoint_learning_eval.py --output /tmp/learning-review.json --generation` with synthetic goals and the intended model environment to exercise the actual verification pipeline. It includes known bad questions, valid controls, and bounded local bank top-offs for LeetCode, MCAT, and LSAT. This does not deploy or exercise AWS queue delivery. The older `checkpoint_question_eval.py capture-bedrock` tool evaluates raw generation and sanitization; it does not establish post-review quality.
