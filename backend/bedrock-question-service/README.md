...
the Nova Lite API remains `legacy`. Roll the worker back with explicit `legacy`;
`inherit` follows the global mode, including a global `native` setting. Default
deployments retain their previous behavior.
