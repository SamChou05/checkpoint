...
Every newly generated question requires an independent solution followed by an answer-blind audit before it enters the bank. The solver sees stems and source/goal scope, with no choices, author key, explanation, level, or previous answer coverage. It returns a short solution, limitations, and explicitly required extra assumptions. Items needing an unstated factual premise are rejected before choices can rescue them. Supported items are reindexed together with their solutions, preserving question identity when other items are removed.
