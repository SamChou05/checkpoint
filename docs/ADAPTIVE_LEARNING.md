# Adaptive learning contract

Checkpoint uses one learning pipeline for arbitrary goals: raw goal → AI-generated skills and objectives → per-skill practice → answer feedback → updated challenge and curriculum. No subject needs a named category, a hand-authored question bank, or a production code branch. Optional source material supplies scope for private, unfamiliar, or user-defined subjects.

Pro practice uses the user's goal and source material as its scope. Each active skill has an independent question difficulty, recent evidence, and objectives needing follow-up. The manual level remains a minimum, not a requirement to accept each automatic increase.

## Evidence and progression

- Use the first answer to each distinct, verified question within the past 30 days. Reported questions and older unverified attempts remain in history but do not drive progression. Repeating a revealed answer does not demonstrate transfer or advance the curriculum.
- Start at the goal's minimum level. Five distinct questions at the current level, at least four correct, and coverage of at least two available objectives advance that skill one level. The ceiling is level 5.
- Three misses in four questions at the current level step that skill back one level, respecting the chosen minimum.
- Recent results identify unresolved objectives and provide bounded question/selected-answer/reference-answer evidence to the AI. Focus Wins and other private notes are excluded.
- The scheduler prefers questions near the skill's target while preserving missed-question review and skill breadth. Replenishment measures coverage at the new target so a large old easy bank cannot prevent harder questions being prepared.
- Bank revisions change with difficulty, skill weights, and unresolved objectives, not on every answer. Requests persist across relaunch, and quizzes continue to use local inventory.
- Curriculum advancement uses recent distinct evidence across every objective, a correct streak, and hard-question evidence. Old lifetime mistakes remain in history without permanently preventing advancement.

## Generation contract

`adaptiveSkillPlans` is an optional array of at most six plans, each scoped to an active skill UUID. Plans carry `targetDifficulty`, recent accuracy/evidence count, up to five focus objective IDs, and up to three recent mistakes. The service validates skill/objective ownership and bounds every learner-provided string. These fields are untrusted evidence, never model instructions.

Generated questions must match their skill's target difficulty. When adaptive plans exist, both the serialized request and the initial/retry prompts derive challenge guidance from those plans; stale goal-wide beginner guidance cannot override an advanced skill. Unplanned skills retain the goal minimum. Without adaptive plans, the original goal guidance remains intact. The worker weights missed objectives more heavily while retaining coverage of the other objectives. Follow-up questions should test the same concept in a new scenario or representation, not reproduce a previously revealed answer.

## Validation boundary

Every newly generated question requires complete-choice independent solving followed by an answer-blind audit before it enters the bank. The solver sees the exact topic, stem and all four choices, plus normalized goal, curriculum and source references. It does not receive the authored key, explanation, difficulty, generated item tags or answer history. Learned facts may be supplied through any subject-reference field; a recall item need not print its answer. The solver must distinguish those facts from unstated case data or lesson intent and assess each choice as an answer to the unchanged item. Exact choice coverage, one declared supported choice, no uncertain choice and agreement with the authored key are required before review. These are checks of model declarations, not a factual certificate. Supported items and solutions are reindexed together, preserving identity when others are removed.

The final reviewer receives stems, rotated choices, independent solutions, goal, skill/objective, source scope, and recent coverage. It never receives the author's key, explanation, or difficulty label. The audit must agree with the authored answer and supply bounded teaching feedback. Its assessed difficulty replaces the author label while respecting the minimum and exact per-skill target. Missing, ambiguous, malformed, or disagreeing reviews reject the question. Duplicate JSON properties and token-truncated responses are rejected. Rejected stems are excluded from subsequent attempts. This remains model-based review, not a correctness guarantee.

The reviewer has its own pinned model configuration (`BEDROCK_VERIFICATION_MODEL_ID`), with Claude Sonnet 4.6 as the local default. It is distinct from the currently evaluated Kimi K2.5 question author. Both prompts apply general quality criteria: explicit assumptions, one defensible answer, scope, exceptions, boundary cases, source support, and conceptual novelty. The same response schema and acceptance rules apply to every subject. There are no goal-name-triggered proof requirements or subject-specific wording filters. Deterministic checks enforce exact answer/choice bounds, reject truncated duplicate choices, and reject feedback referring to shuffled answer letters. Independent model agreement is a quality signal, not proof of correctness.

Generation, independent solving, and final review all count against the same provider-call budget, asynchronous quota, and deadline. No unreviewed output is returned when review runs out of budget. A successful pass normally uses three calls; useful partial results stop before an unaffordable retry. The asynchronous local budget is capped by the durable job allowance. The worker timeout is four minutes and the queue visibility is 24 minutes to accommodate the additional bounded calls. Public HTTP requests retain their existing timeout and may direct users toward asynchronous preparation when insufficient review time remains.

Automated tests exercise longitudinal improvement, independent skill levels, recovery, repeat-answer exclusion, history isolation, request validation, question allocation, and scheduler selection. They establish implementation behavior; they do not establish educational efficacy. Release evaluation must also examine real questions and repeated learner sessions for correctness, conceptual novelty, appropriate challenge, and retention.

## Teaching and rollout

Reviewed questions carry `verificationVersion: 1` and `choiceExplanations`, keyed by exact visible choice text. The quiz shows the selected choice's explanation plus the general reasoning after submission, and saves that feedback in the attempt snapshot for later review. Grading and answer review use the exact verified key; legacy answer-label and explanation heuristics cannot change it. The skill list shows the current adaptive practice level after evidence is available.

Pro switches each goal to reviewed learning after the service first supplies a verified question for that goal. Reviewed attempt history preserves activation even after the cache is pruned. Until then, an older deployed backend remains usable through the legacy workflow. Activated Pro preparation uses a new bank context and claims with `minimumVerificationVersion: 1` and the current `minimumVerificationPolicyRevision` (8). Cached questions below the requested policy are excluded from fresh verified selection; the backend discards old ready inventory and schedules replacements. Claim replay cannot bypass the verification requirement. Free legacy history and question decoding remain compatible.

Deploy and validate the backend, including the reviewer’s explicit invocation permissions, before distributing the updated iOS client. Local test success is not a deployment; an older backend continues to provide the legacy experience, not the new reviewed learning mode. Keep the live correctness evaluation as a release gate.
