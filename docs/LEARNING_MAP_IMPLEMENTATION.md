...
**Release update:** The user approved deployment and needed model calls. The learning-map runtime and deadline correction are deployed; all eight live contract checks pass. Both direct review passes now complete within the HTTP deadline, but generation remains unverified because the drafts were rejected. The asynchronous edit safely superseded the old bank without extra provider calls. The worker-budget correction is verified and committed, with deployment being prepared. The earlier preparation notes below are historical; [the release record](LEARNING_MAP_RELEASE.md) is authoritative for live status.
