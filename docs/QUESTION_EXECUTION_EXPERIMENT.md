...
## Separate output-limit follow-up

After the parser correction (`9a35556`) and runner correction (`14c876c`), a new frozen plan ran **only the previously unattempted output-limit control**. It used clean revision `14c876c`, with a one-session/one-invoke ceiling and the same source, explicit inputs, mode, and harness limits as the original eighth control. The seven completed cases were not repeated. The [separate follow-up archive](evidence/execution-output-followup-20260906.json) contains its manifest, plan, requests, raw decoded result, independent checks, and supervisor-confirmed terminal exit code 0.

One Start, one Invoke, and one Stop completed successfully in **2.495342 seconds** of measured whole-run time. The child attempted to print 4,097 ASCII `x` bytes; its captured text and decoded base64 both contained exactly 4,096 `x` bytes. It reported `truncated=true`, `reason=output_limit`, child exit code zero, and successful child reaping. The session cleanup was `stopped`. There were no provider, transport, parser, or cleanup failures, and billed usage remains unknown.

The observation correctly remained **inconclusive**. The parser separately reported `envelope_valid=true` and `operational_failure=false`, allowing the run to finish with `outcome=completed`. No expected-answer or fixture-specific exemption was needed, and the bounded observation was not promoted into a question rejection or correctness judgment.

The corrected parser also reprocessed the original seventh case's immutable service-result JSON **offline**. It preserved the `child_deadline` observation and classified its envelope as valid with no operational failure. No code execution or AWS call occurred for that reassessment. The original archive is byte-for-byte unchanged: its seven attempts, early stop, failed runner classification, and unattempted eighth case remain historical facts.

All eight fixed controls now have matching observations across the two separately recorded trials: six computational/compiler observations and two bounded limitations. This expands the narrow evidence and cleanup checks; it does not establish MCQ review quality, general program correctness, adversarial isolation, adaptive learning, or production readiness.
