"""Two-call n5 qualification of shared-definition solver transport; semantics separate."""
...
SERVICE = Path("/tmp/checkpoint-solver-shared-schema/backend/bedrock-question-service")
PRIOR = HERE.parent / "solver-identity-qualification-20260922"
sys.path.insert(0, str(PRIOR))
import error_diagnostic as safe_errors  # noqa: E402
...
def expand_owned_schema(schema):
    """Expand only the owned internal definitions for logical equality checks."""
    def expand(value, active=()):
        ...
        if "$ref" in value:
            ...
            ref = value["$ref"]
            assert ref.startswith("#/$defs/") and ref not in active
            return expand(schema["$defs"][ref.split("/")[-1]], (*active, ref))
...
def build_plan():
    ...
    failed = json.loads((PRIOR / "plan.json").read_text())
    jobs = []
    for old in previous["jobs"][:2]:
        ...
        prior_job = failed["jobs"][len(jobs)]
        assert old["items"] == prior_job["items"] and old["case_ids"] == prior_job["case_ids"]
        assert job["request"]["system"][0]["text"].replace("complete_choice_solver_v5_n5", "complete_choice_solver_v4_n5") == prior_job["request"]["system"][0]["text"]
        schema_text = job["request"]["outputConfig"]["textFormat"]["structure"]["jsonSchema"]["schema"]
        prior_schema_text = prior_job["request"]["outputConfig"]["textFormat"]["structure"]["jsonSchema"]["schema"]
        assert expand_owned_schema(json.loads(schema_text)) == json.loads(prior_schema_text)
        job["failed_v4_request_sha256"] = prior_job["request_sha256"]
        job["schema_characters"] = {"v4": len(prior_schema_text), "v5": len(schema_text)}
    ...
    return {"experiment": "Solver v5 shared-definition n5 structural qualification; semantics scored separately",
            "source_sha256": sources,
            "new_source_sha256": {name: sha(HERE / name) for name in ("PLAN.md", "solver_shared_schema_probe.py", "test_solver_shared_schema_probe.py")},
            ...
            "prior_evidence_sha256": {name: sha(PRIOR / name) for name in ("plan.json", "capture.json", "error_diagnostic.py", "error-diagnostic-plan.json", "error-diagnostic-capture.json")},
            ...
            "claim_limits": "Repeated diagnostics and n5-only structural qualification. Other counts require separate service qualification. No semantic determinism, broad accuracy, causal effect or worker-yield claim. Prior failures unchanged; no production semantic prompt, policy or model settings changed."}
...
def run_calls(client, plan, capture, path, *, sensitive_values=()):
    if len(plan["jobs"]) != 2 or plan["settings"]["maximum_calls"] != 2 or capture["calls"]:
    ...
    for job in plan["jobs"]:
        ...
        try:
            ...
            row["safe_error"] = safe_errors.safe_error_details(error, sensitive_values=sensitive_values)
