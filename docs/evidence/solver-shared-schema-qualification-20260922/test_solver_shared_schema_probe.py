...
class SolverSharedSchemaProbeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_shared_definitions_expand_to_failed_v4_logical_schema(self):
        previous = json.loads((probe.PRIOR / "plan.json").read_text())
        for job, old in zip(self.plan["jobs"], previous["jobs"], strict=True):
            schema = json.loads(job["request"]["outputConfig"]["textFormat"]["structure"]["jsonSchema"]["schema"])
            old_schema = json.loads(old["request"]["outputConfig"]["textFormat"]["structure"]["jsonSchema"]["schema"])
            self.assertEqual(probe.expand_owned_schema(schema), old_schema)
            self.assertEqual(set(schema["$defs"]), {"choiceJudgment", "pairRelation", "solution"})
            self.assertEqual(list(schema["properties"]["solutions"]["properties"]), list(map(str, range(5))))
            self.assertEqual(list(schema["$defs"]["choiceJudgment"]["properties"]), ["reason", "judgment"])
            self.assertEqual(list(schema["$defs"]["pairRelation"]["properties"]), ["reason", "relation"])
            self.assertEqual(job["schema_characters"], {"v4": 11798, "v5": 1489})
            ...
            self.assertEqual(comparison, old["request"])
        self.assertEqual(self.plan["native_contract"]["name"], "complete_choice_solver_v5_n5")

    def test_provider_failure_retains_only_bounded_redacted_safe_details(self):
        secret = "SYNTHETIC_PRIVATE_CREDENTIAL"
        class Failure(Exception):
            response = {"Error": {"Code": "ValidationException", "Message": "Synthetic " + secret + " failure"},
                        "ResponseMetadata": {"HTTPStatusCode": 400, "RequestId": "synthetic-request", "HTTPHeaders": {"private": secret}}}
        def fail(_):
            raise Failure("Do not serialize exception arguments " + secret)
        client, capture = Client(fail), {"calls": []}
        probe.run_calls(client, self.plan, capture, Path("unused"), sensitive_values=(secret,))
        self.assertEqual(len(client.calls), 1)
        self.assertNotIn(secret, json.dumps(capture))
        self.assertNotIn("HTTPHeaders", json.dumps(capture))
        self.assertEqual(capture["calls"][0]["safe_error"], {
            "Error": {"Code": "ValidationException", "Message": "Synthetic [REDACTED] failure"},
            "HTTPStatus": 400, "RequestId": "synthetic-request"})
