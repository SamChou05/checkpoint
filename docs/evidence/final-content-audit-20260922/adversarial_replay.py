"""No-network replay and mechanical denominator summary for the separate trial."""
...
def main():
    ...
    with patch.object(original.boto3, "client", side_effect=AssertionError("No network")), patch.object(
        ...
        for row in capture["calls"]:
            ...
            result = {"sequence": row["sequence"], "source_sequence": call.get("source_sequence"), "exact_request": True}
            if "assessment" in row:
                assert original.assess(copy.deepcopy(row["response"]), call, plan) == row["assessment"]
                result["strict_native_and_immutable_admission_replay"] = True
            elif row.get("validation_error_type"):
                try:
                    original.assess(row["response"], call, plan)
    ...
    result = {"capture_sha256": original.sha(probe.CAPTURE.read_bytes()), "plan_sha256": capture["plan_sha256"],
              ...
              "source_failed_sonnet_capture_sha256": plan["source_failed_sonnet_capture_sha256"], "checks": checks,
              "status": capture["status"], "planned_calls": 4, "attempted_calls": len(capture["calls"]),
              "unattempted_calls": 4 - len(capture["calls"]), "planned_items": 24, "strict_assessed_items": len(items),
              "failed_or_unattempted_items": 24 - len(items),
              "exact_dispositions": sum(item["matching_disposition"] for item in items),
              "sound_accepted_of_12": sum(item["verdict"] == item["gold_verdict"] == "accepted" for item in items),
              "defective_rejected_of_12": sum(item["verdict"] == item["gold_verdict"] == "rejected" for item in items),
              ...
              "case_results": items}
    audit_path = probe.HERE / "independent-adversarial-output-audit.json"
    if audit_path.exists():
        result["independent_audit_sha256"] = original.sha(audit_path.read_bytes())
    original.save(probe.HERE / "adversarial-replay-summary.json", result)
