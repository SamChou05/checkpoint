...
def main():
    ...
    with patch.object(original.boto3, "client", side_effect=AssertionError("No network")), patch.object(
        ...
        for row in capture["calls"]:
            ...
            result = {"sequence": row["sequence"], "source_sequence": row["source_sequence"], "exact_request": True}
    ...
    result = {"capture_sha256": original.sha(probe.CAPTURE.read_bytes()), "plan_sha256": capture["plan_sha256"],
              ...
              "original_failed_capture_sha256": plan["source_failed_capture_sha256"], "checks": checks,
              "status": capture["status"], "planned_calls": 3, "attempted_calls": len(capture["calls"]),
              "unattempted_calls": 3 - len(capture["calls"]), "planned_items": 18, "strict_assessed_items": len(items),
              "failed_or_unattempted_items": 18 - len(items),
              "exact_dispositions": sum(item["matching_disposition"] for item in items),
              "sound_accepted_of_9": sum(item["verdict"] == item["gold_verdict"] == "accepted" for item in items),
              "defective_rejected_of_9": sum(item["verdict"] == item["gold_verdict"] == "rejected" for item in items),
              ...
              "case_results": items}
    audit_path = probe.HERE / "independent-adaptive-only-output-audit.json"
    if audit_path.exists():
        result["independent_audit_sha256"] = original.sha(audit_path.read_bytes())
    original.save(probe.HERE / "adaptive-only-replay-summary.json", result)
