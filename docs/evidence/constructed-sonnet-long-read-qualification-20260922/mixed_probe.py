"""Unfrozen longer-read Sonnet constructed-task actual-route qualification; import and preflight make no calls."""
...
ROOT = HERE.parents[2]
SERVICE = Path("/tmp/checkpoint-worker-response-timeout-200/backend/bedrock-question-service").resolve()
ORIGINAL_JOBS = ROOT / "docs/evidence/quantitative-mixed-qualification-20260922/jobs-draft.json"
BASELINE_PLAN = ROOT / "docs/evidence/constructed-sonnet-worker-qualification-20260922/plan.json"
BASELINE_CAPTURE = BASELINE_PLAN.with_name("capture.json")
BASELINE_PLAN_SHA256 = "6230622117349a2617e7502def86d625bd70fb401b82c3dd8f59c2a048624dcc"
BASELINE_CAPTURE_SHA256 = "9402ce571dee1c386aa3489e548b82292408e9b4a984a4807d596a6ca982a707"
AUTHOR_MODEL = "us.anthropic.claude-sonnet-4-6"
SOURCE_COMMIT = "77f794efa8cc24f94123cda61532ae7fe30d3e38"
...
ENVIRONMENT = {
    ...
    "BEDROCK_CONNECT_TIMEOUT_SECONDS": "3", "BEDROCK_READ_TIMEOUT_SECONDS": "200",
...
LIMITS = {"jobs": 3, "questions_per_job": 5, "maximum_calls": 18, "calls_per_job": 6,
          "seconds_per_job": 240, "read_ceiling": 200, "connect_timeout": 3, "sdk_attempts": 1}
...
def allowed_generation_source(baseline_text):
    """Exact allowed source delta; never normalize arbitrary source changes."""
    before = ('            "BEDROCK_READ_TIMEOUT_SECONDS",\n'
              '            DEFAULT_BEDROCK_READ_TIMEOUT_SECONDS,\n'
              '            MIN_BEDROCK_READ_TIMEOUT_SECONDS,\n'
              '            100.0,')
    after = before.replace('100.0,', '200.0,')
    old_comment = "        # Author, solver, and reviewer share the HTTP Lambda's 30-second limit."
    new_comment = ("        # All stages share their Lambda's deadline (30 seconds for the API,\n"
                   "        # 240 seconds for the worker), even with a longer configured ceiling.")
    require(baseline_text.count(before) == 1 and baseline_text.count(old_comment) == 1,
            "Baseline timeout source no longer matches the approved delta.")
    return baseline_text.replace(before, after).replace(old_comment, new_comment)


def check_source_delta(baseline_service, modules):
    """Bind both source trees and allow only the reviewed timeout adjustment."""
    old_modules = sorted(baseline_service.glob("*.py"))
    require([path.name for path in old_modules] == [path.name for path in modules],
            "Candidate runtime module set changed.")
    changed = []
    for path in modules:
        old = baseline_service / path.name
        if path.name == "question_generation.py":
            require(path.read_bytes() == allowed_generation_source(old.read_bytes().decode("utf-8")).encode("utf-8"),
                    "Candidate generation source exceeds approved timeout delta.")
        else:
            require(path.read_bytes() == old.read_bytes(), "Unrelated candidate runtime module changed.")
        if path.read_bytes() != old.read_bytes():
            changed.append(path.name)
    for relative in ("requirements.txt", "scripts/validate-native-sdk.py", "evals/bounded_bedrock_capture.py"):
        require((SERVICE / relative).read_bytes() == (baseline_service / relative).read_bytes(),
                "Candidate helper or dependency changed.")
    return {"candidate_commit": SOURCE_COMMIT, "baseline_source_root": str(baseline_service),
            "changed_modules": changed, "allowed_transform": "Only configured read upper bound100 to200 and explanatory deadline comment; exact source transform enforced"}
...
def build_plan():
    ...
    require(baseline["state"] == "frozen" and len(baseline["source_hashes"]) == 44,
            "Expected frozen baseline with all 44 source pins.")
    ...
    require(jobs == baseline["jobs"] and LIMITS == {**baseline["limits"], "read_ceiling": 200}
            and ENVIRONMENT == {**baseline["environment"], "BEDROCK_READ_TIMEOUT_SECONDS": "200"},
            "Baseline jobs, budgets or settings beyond the read ceiling changed.")
    source_delta = check_source_delta(Path(baseline["source_root"]), modules)
    require(source_delta["changed_modules"] == ["question_generation.py"],
            "Unexpected candidate source change.")
    ...
    return {"state": "draft", "source_root": str(SERVICE), "source_change": source_delta, "jobs": jobs, "environment": ENVIRONMENT,
            ...
            "reused_scopes": {"original_jobs_path": str(ORIGINAL_JOBS), "original_jobs_sha256": file_hash(ORIGINAL_JOBS),
                              ...
                              "changes": ["Read ceiling only: environment100 to200 and runtime configuration upper bound100 to200, with deadline comment clarification"],
                              "unchanged": ["all other runtime bytes", "task-only author prompt/schema", "all models and reasoning settings", "three jobs", "all prospective criteria", "240-second deadline and six-call job cap"]},
            "claim_limits": "Three newly generated candidate batches on repeated scopes, not fresh-domain generalization, a matched causal comparison or cold-cache attribution, arbitrary-topic accuracy or deployed worker/inventory qualification; independent learner-content adjudication required."}
