...
SPEC = importlib.util.spec_from_file_location("constructed_sonnet_long_read_probe_tested", HERE / "mixed_probe.py")
...
class MixedProbeTests(unittest.TestCase):
    ...
    def test_three_real_mixed_jobs_use_eight_calls_and_preserve_compiler_fields(self):
        ...
        self.assertEqual(len(probe.IMPORTED_SERVICE_HASHES), 27)
    ...
    def test_only_read_ceiling_and_allowed_source_change_all_baseline_pins_remain_active(self):
        baseline = probe.safe.strict_json(probe.BASELINE_PLAN.read_text())
        self.assertEqual(probe.file_hash(probe.BASELINE_PLAN), probe.BASELINE_PLAN_SHA256)
        self.assertEqual(probe.file_hash(probe.BASELINE_CAPTURE), probe.BASELINE_CAPTURE_SHA256)
        self.assertEqual(len(baseline["source_hashes"]), 44)
        self.assertEqual({k for k in self.plan["environment"] if self.plan["environment"][k] != baseline["environment"][k]}, {"BEDROCK_READ_TIMEOUT_SECONDS"})
        self.assertEqual(self.plan["environment"]["BEDROCK_MODEL_ID"], probe.AUTHOR_MODEL)
        self.assertEqual(self.plan["environment"]["BEDROCK_READ_TIMEOUT_SECONDS"], "200")
        self.assertEqual(self.plan["limits"], {**baseline["limits"], "read_ceiling": 200})
        self.assertEqual(self.plan["source_change"]["changed_modules"], ["question_generation.py"])
        self.assertEqual(self.plan["source_change"]["candidate_commit"], probe.SOURCE_COMMIT)
        for field in ("jobs", "endpoint", "contracts", "initial_author_prompts", "criteria", "credential_limits", "failure_policy", "dependencies"):
            self.assertEqual(self.plan[field], baseline[field], field)
        for path, sha in baseline["source_hashes"].items():
            self.assertEqual(self.plan["source_hashes"][path], sha)
        self.assertIn(str(probe.BASELINE_CAPTURE), self.plan["source_hashes"])
        original_hash = probe.file_hash
        for target in (probe.BASELINE_PLAN, probe.BASELINE_CAPTURE, probe.BASELINE_PLAN.with_name("PLAN.md")):
            with self.subTest(target=target), patch.object(probe, "file_hash", side_effect=lambda path: "bad" if path == target else original_hash(path)):
                with self.assertRaises(probe.IntegrityError):
                    probe.check_plan(self.plan)

    def test_only_exact_approved_timeout_source_transform_is_allowed(self):
        baseline = probe.safe.strict_json(probe.BASELINE_PLAN.read_text())
        old = Path(baseline["source_root"]) / "question_generation.py"
        actual = probe.SERVICE / "question_generation.py"
        self.assertEqual(probe.allowed_generation_source(old.read_text()), actual.read_text())
        for bad in (old.read_text().replace("            100.0,", "            101.0,"),
                    old.read_text() + old.read_text()):
            with self.subTest(bad_length=len(bad)), self.assertRaises(probe.IntegrityError):
                probe.allowed_generation_source(bad)
        ...
        for transform in (lambda value: value + b"\n# unauthorized extra change\n",
            ...
            with patch.object(Path, "read_bytes", changed), self.assertRaises(probe.IntegrityError):
                probe.check_source_delta(Path(baseline["source_root"]), sorted(probe.SERVICE.glob("*.py")))
