...
SPEC = importlib.util.spec_from_file_location("constructed_sonnet_worker_probe_tested", HERE / "mixed_probe.py")
...
class MixedProbeTests(unittest.TestCase):
    ...
    def test_only_configured_author_model_changes_and_all_baseline_pins_remain_active(self):
        ...
        self.assertEqual(len(baseline["source_hashes"]), 37)
        self.assertEqual({k for k in self.plan["environment"] if self.plan["environment"][k] != baseline["environment"][k]}, {"BEDROCK_MODEL_ID"})
        self.assertEqual(self.plan["environment"]["BEDROCK_MODEL_ID"], probe.AUTHOR_MODEL)
        for field in ("source_root", "jobs", "limits", "endpoint", "contracts", "initial_author_prompts", "criteria", "credential_limits", "failure_policy", "dependencies"):
