# Draft qualification of compiled proofs and immutable-main review
...
the blind solver retains its vetoes for prose. Exact privately revalidated compiler rows bypass only the redundant model correctness/pair solver. The immutable-main auditor still reviews every survivor for scope, difficulty, novelty, plausible distractors, key agreement and supported teaching. Their output cannot replace compiled
teaching or ordinary authored main explanations. Other subjects keep the prose
route with an empty choice-feedback map and the existing client main fallback.
Invalid numerical specifications are rejected, never silently repaired or
interpreted as prose.

`jobs-draft.json` copies the previous mixed-route trial's exact three normalized
requests for five questions each: exact rational arithmetic and explicit bounded conditions;
...
The scopes are repeated, not a fresh holdout. This trial changes the author model/settings, worked compiler mains, final audit contract and compiled solver routing together; it cannot isolate
the causal effect of any one change. The original jobs file is pinned and compared
byte-for-byte; no previous failed result is replaced or reinterpreted.
...
`QUESTION_FEEDBACK_CONTRACT=authored_solution`, with Kimi K2.5 for authoring (thinking disabled, 6,000 tokens, temperature 0.2), and Sonnet 4.6 for prose solving and all final audits (adaptive/high, 16,000 tokens, no sampling fields). The actual selected contracts are mixed author v1,
solver v5 count 1–5 and immutable-main issue-flag audit v3 count 1–5. The author receives
the actual authored-main system prompt, which differs from the earlier trial.
No fallback model, guardrail override or production environment changes. The candidate's default
...
- Every returned item has one correct exact key, distinct and plausible choices,
  ...
  difficulty, and supported main explanation plus every supplied choice
  explanation. Prose must return empty choice feedback, which is valid; compiler items retain all
  four derived choice explanations. Uncertainty in released learner content fails
  the item. Private diagnostic reasons are reported separately.
- Each returned compiled item's exact five learner fields reproduce from its
  ...
  Ordinary prose main text must exactly match its author source and remain
  unchanged after sanitization, with policy 7 and empty feedback. Compiled items
  use distinct policy 8, identifying compiler proof plus final independent audit; historical policy 6 retains its independent model solver promise. No provider flag or content hash creates compiler provenance.
...
The observer records actual per-pass verifier outputs and retains those objects
for final identity binding. It never searches all attempts for equal learner
content. Compiled sources retain their original private-sidecar raw ordinal;
prose observation IDs use the unique sanitized ordinal in that pass, with exact
raw author-main bindings recorded separately. Identical raw duplicates may have
more than one matching raw ordinal without inventing a unique provenance claim.
Fake tests preserve actual rejected-prompt deduplication and also repeat a raw
surplus candidate across passes without misattributing its eventual release.
Freeze and execution create exclusive files; an existing capture cannot resume.

A passing trial would support only the tested candidate and these three newly
generated batches on repeated scopes. It cannot establish an error rate for arbitrary topics, deterministic model
