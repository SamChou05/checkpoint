"""Unfrozen compiled-proof actual-route qualification; import and preflight make no calls."""
...
ROOT = HERE.parents[2]
SERVICE = Path("/Users/samchou/.codex/worktrees/worked-compiled-worker-source/Checkpoint/backend/bedrock-question-service").resolve()
...
ENVIRONMENT = {
    ...
    "QUESTION_AUTHOR_MODE": "mixed_quantitative", "QUESTION_FEEDBACK_CONTRACT": "authored_solution",
...
def build_plan():
    ...
    paths = [*modules, SERVICE / "requirements.txt", SERVICE / "scripts/validate-native-sdk.py",
             HELPER, ORIGINAL_JOBS, HERE / "mixed_probe.py", HERE / "test_mixed_probe.py", HERE / "PLAN.md", HERE / "jobs-draft.json"]
    contracts = [MIXED_AUTHOR_CONTRACT, *[kind(count) for kind in (native.SolverSlotContract, native.AuthoredSolutionFlagReviewContract)
    ...
    return {"state": "draft", "source_root": str(SERVICE), "jobs": jobs, "environment": ENVIRONMENT,
            ...
            "criteria": {"returned_total_of_15": 14, "minimum_per_job": 4, "compiled_total": 6,
                         ...
                         "prose_main_exactly_authored": True, "prose_feedback_must_be_empty": True,
            ...
            "reused_scopes": {"original_jobs_path": str(ORIGINAL_JOBS), "original_jobs_sha256": file_hash(ORIGINAL_JOBS),
                              ...
                              "changes": ["Kimi K2.5 disabled author", "authored-main system prompt",
                                          "worked deterministic arithmetic mains", "immutable-main native-v3 flags",
                                          "compiler-proven rows skip model solver; final audit retained", "compiled8 and prose7 release"]},
    ...
    class Recorder:
        ...
        def converse(self, **request):
            ...
            guard(request == {"modelId": ENVIRONMENT["BEDROCK_MODEL_ID" if author else "BEDROCK_VERIFICATION_MODEL_ID"],
                ...
                "additionalModelRequestFields": ({"thinking": {"type": "disabled"}} if author else {"thinking": {"type": "adaptive"}, "output_config": {"effort": "high"}})}, "Actual request drift.")
