"""Unfrozen constructed-task actual-route qualification; import and preflight make no calls."""
...
BASELINE_PLAN = ROOT / "docs/evidence/compiled-proof-worker-qualification-20260922/plan.json"
...
def build_plan():
    ...
    require(jobs == baseline["jobs"] and LIMITS == baseline["limits"]
            and ENVIRONMENT == {**baseline["environment"], "QUESTION_AUTHOR_MODE": "constructed_quantitative"},
            "Baseline jobs, budgets or non-author-mode settings changed.")
    paths = [*modules, SERVICE / "requirements.txt", SERVICE / "scripts/validate-native-sdk.py",
             ...
             HERE / "PLAN.md", HERE / "jobs-draft.json", HERE / "make_blind_review.py"]
    ...
    require([item["user"] for item in initial] == [item["user"] for item in baseline["initial_author_prompts"]],
            "Initial author task inputs changed.")
    require(all(native.native_output_config(contract) == baseline["contracts"][native.contract_metadata(contract)["name"]]
                for contract in contracts if contract != CONSTRUCTED_AUTHOR_CONTRACT),
            "Prose solver or immutable auditor contracts changed.")
    return {"state": "draft", "source_root": str(SERVICE), "jobs": jobs, "environment": ENVIRONMENT,
            ...
            "reused_scopes": {"original_jobs_path": str(ORIGINAL_JOBS), "original_jobs_sha256": file_hash(ORIGINAL_JOBS),
                              ...
                              "changes": ["task-only numerical author native contract and instructions",
                                          "deterministic exact numerical option construction"]},
