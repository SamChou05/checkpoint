"""Summarize the completed fixed-slot trial without model calls or repairs."""
import hashlib
import json
...
def main():
    plan = json.loads((HERE / "slot-plan.json").read_text())
    capture = json.loads((HERE / "slot-capture.json").read_text())
    assert capture["status"] in {"complete", "stopped_after_failure"}
    assert capture["plan_byte_sha256"] == sha(HERE / "slot-plan.json")
    ...
    result = {
        "plan_byte_sha256": sha(HERE / "slot-plan.json"), "capture_byte_sha256": sha(HERE / "slot-capture.json"),
        ...
        "limits": "One sample per batch on 20 reused reviewed cases. Combined fixed identifiers, key-independent slot order and reason-first intervention, not a causal isolation or general accuracy/determinism result. Earlier failures unchanged.",
    }
    (HERE / "slot-summary.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n")
