"""Freeze/run four trusted-endpoint solver qualification calls; no retries."""
import argparse
...
def source_files():
    return {str(path.relative_to(base.REPO)): base.sha(path) for path in [
        ...
        HERE / "slot_original_complete_question_solution.py",
    ]}
...
def make_plan():
    ...
    previous_slot = json.loads((HERE / "slot-plan.json").read_text())
    jobs = []
    for prior in original["jobs"]:
        ...
        assert request["outputConfig"] == previous_slot["jobs"][job["call_index"]]["request"]["outputConfig"]
        for item in slots_from_request(job):
            assert set(item["choicePairs"]) == set(candidate.CHOICE_PAIR_SLOTS)
    ...
    return {
        ...
        "prior_slot_capture_sha256": base.sha(HERE / "slot-capture.json"),
        "prior_slot_plan_sha256": base.sha(HERE / "slot-plan.json"),
        ...
        "scope": "Four candidate calls on unchanged twenty reviewed reused controls. Joint addition of trusted input pair endpoints and explicit requested-meaning distinction; same v3 schema, key-independent slot order and strict decoder. No baseline/author calls or single-change causal claim; previous failures unchanged.",
...
def assert_bindings(plan):
    ...
    paths = {"runner_sha256": Path(__file__), "document_sha256": HERE / "ENDPOINT_PLAN.md",
             "prior_qualification_plan_sha256": HERE / "plan.json", "prior_qualification_capture_sha256": HERE / "capture.json",
             "prior_slot_capture_sha256": HERE / "slot-capture.json", "prior_slot_plan_sha256": HERE / "slot-plan.json",
...
def main():
    ...
    assert_bindings(plan)
    output = HERE / "endpoint-capture.json"
    if output.exists():
