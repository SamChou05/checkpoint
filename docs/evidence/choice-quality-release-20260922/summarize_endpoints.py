"""Summarize the completed endpoint/meaning trial without model calls or repairs."""
import hashlib
import json
...
def main():
    plan = json.loads((HERE / "endpoint-plan.json").read_text())
    capture = json.loads((HERE / "endpoint-capture.json").read_text())
    assert capture["status"] in {"complete", "stopped_after_failure"}
    assert capture["plan_byte_sha256"] == sha(HERE / "endpoint-plan.json")
    ...
    result = {
        "plan_byte_sha256": sha(HERE / "endpoint-plan.json"), "capture_byte_sha256": sha(HERE / "endpoint-capture.json"),
        ...
        "per_call": [{"call_index": call["call_index"], "stop_reason": call.get("stop_reason"), "error": call.get("error"), "elapsed_seconds": call.get("elapsed_seconds"), "usage": call.get("usage"), "all_gold_agrees": sum(row["all_gold_agrees"] for row in call.get("score", {}).get("rows", []))} for call in capture["calls"]],
        "limits": "One sample per batch on 20 reused reviewed cases. Joint trusted pair endpoints and requested-meaning intervention with unchanged fixed-slot schema, not a causal isolation or general accuracy/determinism result. Earlier failures unchanged.",
    }
    (HERE / "endpoint-summary.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n")
