"""Five-call Opus 4.7 JSON-prompted audit diagnostic; no account activation."""
...
safe = load("opus47_safe_worker", "authored-feedback-worker-20260922/worker_pipeline_probe.py")
controls = load("opus47_control_source", "authored-feedback-combined-controls-20260922/combined_probe.py")
...
def build_plan():
    ...
    paths = [HERE / name for name in ("opus47_probe.py", "test_opus47_probe.py", "PLAN.md", "access-evidence.json")]
    paths.extend([Path(safe.__file__), Path(controls.__file__), *sorted(evidence)])
    return {"plan_state": "draft_unapproved", "experiment": "Fixed Opus4.7 final-audit-only known-control diagnostic",
            ...
            "access_evidence": safe.strict_json((HERE / "access-evidence.json").read_text()),
            "capability_evidence": {
                "model_card": "https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-anthropic-claude-opus-4-7.html",
                "adaptive_thinking": "https://docs.aws.amazon.com/bedrock/latest/userguide/claude-messages-adaptive-thinking.html",
                "api": "Bedrock Runtime Converse", "inference_profile": MODEL,
                "native_structured_output": "unsupported", "sampling_parameters": "unsupported; omitted",
                "thinking": "adaptive", "effort": "high in additionalModelRequestFields.output_config",
            },
            "access_condition": "Read-only status evidence reports available access and an active US profile; no account activation is implemented. Dispatch requires root review and the exact frozen plan hash. Status availability does not itself prove inference success.",
            "usage_bound": "At most 80000 shared output tokens across five requests. Input and output usage are reported separately; unavailable timeout usage is not zero. No dollar cap or unverified model price is claimed.",
...
def preflight():
    result = subprocess.run([sys.executable, "-B", "-m", "unittest", "discover", "-s", str(HERE),
                             "-p", "test_opus47_probe.py", "-q"], capture_output=True, text=True, check=False)
...
def execute(expected_hash, *, plan_path=PLAN, capture_path=CAPTURE):
