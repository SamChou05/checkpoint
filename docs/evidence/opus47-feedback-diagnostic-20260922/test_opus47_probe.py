...
SPEC = importlib.util.spec_from_file_location("opus47_diagnostic_tested", HERE / "opus47_probe.py")
...
class DiagnosticTests(unittest.TestCase):
    ...
    def test_all24_full_originals_gold_exact_scope_and_sdk_requests(self):
        ...
        self.assertEqual(probe.MODEL, "us.anthropic.claude-opus-4-7")
        self.assertNotIn("commercial_review", self.plan)
        access = self.plan["access_evidence"]
        self.assertEqual(access["foundation_model"], {
            "modelId": "anthropic.claude-opus-4-7", "agreementStatus": "AVAILABLE",
            "authorizationStatus": "AUTHORIZED", "entitlementAvailability": "AVAILABLE",
            "regionAvailability": "AVAILABLE"})
        self.assertEqual(access["inference_profile"]["inferenceProfileId"], probe.MODEL)
        self.assertEqual(access["inference_profile"]["status"], "ACTIVE")
        self.assertEqual(access["inference_calls"], 0)
        self.assertEqual(access["account_mutations"], 0)
    ...
    def test_execute_requires_frozen_plan_and_rejects_config_before_credentials(self):
        path = Path(self.directory.name) / "plan.json"
    ...
    def test_execute_fake_session_and_exclusive_capture(self):
        ...
        with patch.object(probe.safe, "credential_session", return_value=(session, ())):
            result = probe.execute(probe.file_hash(path), plan_path=path, capture_path=self.path)
