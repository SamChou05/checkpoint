...
SPEC = importlib.util.spec_from_file_location("immutable_main_mixed_probe_tested", HERE / "mixed_probe.py")
    ...
    def converse(self, **request):
        ...
        if name == probe.MIXED_AUTHOR_CONTRACT:
            ...
            payload = {"reviews": {str(item["index"]): {
                ...
                "explanationSupport": "supported", "issues": []} for item in given["items"]}}
...
class MixedProbeTests(unittest.TestCase):
    ...
    def test_solver_filtering_reindexes_review_and_topup_preserves_original_sources(self):
        ...
        self.assertEqual(reviewer["stage"], probe.native.contract_metadata(probe.native.AuthoredSolutionReviewContract(3))["name"])
    ...
    def test_repeated_rejected_content_obeys_actual_topup_deduplication(self):
        ...
        self.run_fake(FakeNetwork(modify))
        first = self.capture["jobs"][0]
        self.assertEqual((first["provider_calls"], len(first["returned"])), (4, 4))
        self.assertEqual(first["passes"][1]["sanitized"], [])
        self.assertEqual(first["passes"][1]["verified"], [])
        self.assertTrue(all(source[1] == 0 for source in first["returned_sources"]))
        self.assertFalse(self.capture.get("global_stop"))
        ...
        for invalid in ("replacement", "support", "issues"):
            ...
            def modify(index, request, payload, response):
                if index == 2:
                    if invalid == "replacement":
                        ...
                        payload["reviews"]["0"]["issues"] = ["The supplied objective is not satisfied."]
                    response["output"]["message"]["content"] = [{"text": json.dumps(payload)}]
                return response
            with self.subTest(invalid=invalid):
                ...
                if invalid == "replacement":
                    ...
                    self.assertEqual(first["provider_calls"], 3)
