...
ENVIRONMENT = {
    ...
    "BEDROCK_MODEL_ID": "us.anthropic.claude-sonnet-4-6", "BEDROCK_VERIFICATION_MODEL_ID": "us.anthropic.claude-sonnet-4-6",
...
def build_plan():
    ...
    contracts = [MIXED_AUTHOR_CONTRACT, *[kind(count) for kind in (native.SolverSlotContract, native.AuthoredSolutionReviewContract)
    ...
    return {"state": "draft", "source_root": str(SERVICE), "jobs": jobs, "environment": ENVIRONMENT,
            ...
            "reused_scopes": {"original_jobs_path": str(ORIGINAL_JOBS), "original_jobs_sha256": file_hash(ORIGINAL_JOBS),
                              ...
                              "changes": ["Sonnet adaptive-high author", "authored-main system prompt",
                                          "immutable-main count-bound audit", "compiled6 and prose7 release"]},
    ...
    def verify(questions, *args, **kwargs):
        ...
        for question in returned:
            ...
            if source is not None:
                guard(question.get("verificationPolicyRevision") == 6 and learner(question) == source["learner"],
