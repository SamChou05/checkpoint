"""Prospective scoped-audit candidate on unchanged learner controls; no inference in preparation."""
...
PREVIOUS = HERE.parent / 'authored-feedback-qualification-20260922'
MAX_CALLS = 4
COUNT = 6
...
def compact_case(case):
    return {**{key: copy.deepcopy(case[key]) for key in ('case_id', 'batch', 'slot', 'learner_item', 'learner_content_sha256')},
            'gold': {'required_gate_decision': case['gold']['required_gate_decision']}}


def compact_expectation(row):
    return {'case_id': row['case_id'], 'original_required_gate_decision': row['original_required_gate_decision'],
            'task': {key: copy.deepcopy(row['task'][key]) for key in ('expected_judgment', 'proof')},
            'answer_set': {'expected_answerChoice': row['answer_set']['expected_answerChoice']},
            'feedback': {slot: {key: copy.deepcopy(value[key]) for key in ('expected_judgment', 'proof')}
                         for slot, value in row['feedback'].items()}}
...
def build_plan():
    ...
    for sequence, previous in enumerate(old['calls']):
        ...
        data = audit.build_input(originals, scope, assignments=[{} for _ in originals], history=[])
        prompt = '<authored_feedback_audit_json>\n' + json.dumps(data, ensure_ascii=False, allow_nan=False) + '\n</authored_feedback_audit_json>'
        assert prompt == audit.build_user_prompt(originals, scope, assignments=[{} for _ in originals], history=[])
        request = {
            'modelId': MODEL,
    ...
    for module in (audit, author, verification, native, generation):
    ...
    result = {
        ...
        'experiment': 'Scope-aware immutable-feedback candidate with 100-second limit on 24 known controls',
        'runtime_source_sha256': source_pins,
        'harness_source_sha256': {name: sha(HERE / name) for name in ('PLAN.md', 'audit_scoped.py', 'test_audit_scoped.py')},
        ...
        'primary_prospective_criteria': {'structurally_valid_calls': 4, 'all_end_turn': True,
                                       'each_provider_call_at_most_seconds': 100, 'correct_admission_decisions': 24,
        ...
        'failure_policy': 'Attempt all four fixed independent batches once. Provider timeouts/errors, non-end_turn, elapsed-over-100 and malformed native/local output fail only that batch; continue unchanged next inputs without retries, rescue or replacement. Stop globally on source/request/hash drift, credentials/setup or budget/content-integrity failure. Every failed batch earns zero credit; all4/24/144 denominators and all passing criteria remain unchanged.',
    ...
    failed = json.loads((PREVIOUS / 'capture.json').read_text())
    assert sha(PREVIOUS / 'plan.json') == failed['plan_sha256']
    assert failed['status'] == 'stopped_after_failure' and len(failed['calls']) == 1
    assert failed['calls'][0]['provider_error']['type'] == 'ReadTimeoutError'
    assert failed['calls'][0]['request'] == previous['calls'][0]['request']
    for key in ('dependencies', 'cases', 'field_expectations', 'diagnostic_reporting'):
        assert result[key] == previous[key], key
    for call, old_call in zip(result['calls'], previous['calls'], strict=True):
        assert call['case_ids'] == old_call['case_ids']
        assert call['scope'] == old_call['scope']
        assert call['learner_content_sha256'] == old_call['learner_content_sha256']
        for key in ('modelId', 'inferenceConfig', 'additionalModelRequestFields'):
            assert call['request'][key] == old_call['request'][key]
        after_data, before_data = tagged_data(call['request']), tagged_data(old_call['request'])
        assert after_data.pop('existingQuestionCoverage') == []
        for item in after_data['items'].values():
            assert item.pop('assignment') == {}
        assert after_data == before_data
        after_schema = call['request']['outputConfig']['textFormat']['structure']['jsonSchema']
        before_schema = old_call['request']['outputConfig']['textFormat']['structure']['jsonSchema']
        assert after_schema['schema'] == before_schema['schema']
        assert set(after_schema) == set(before_schema) == {'name', 'schema'}
        call['prior_v2_request_sha256'] = old_call['request_sha256']
        call['changed_request_fields'] = [key for key in call['request'] if call['request'][key] != old_call['request'][key]]
        assert set(call['changed_request_fields']) <= {'system', 'messages', 'outputConfig'}
    old_limits = {**result['limits'], 'read_timeout_seconds': 75}
    old_criteria = {**result['primary_prospective_criteria'], 'each_provider_call_at_most_seconds': 75}
    assert old_limits == previous['limits'] and old_criteria == previous['primary_prospective_criteria']
    result['prior_trial_sha256'] = {name: sha(PREVIOUS / name) for name in ('plan.json', 'capture.json', 'audit_qualification.py', 'test_audit_qualification.py')}
    result['candidate_changes'] = {'scope_aware_prompt_and_inputs': True,
                                   'known_controls_assignment_and_history_context_empty': True,
                                   'read_timeout_seconds': {'old': 75, 'new': 100},
                                   'elapsed_criterion_seconds': {'old': 75, 'new': 100},
                                   'runtime_read_ceiling_seconds': 100,
                                   'future_worker_deadline_seconds': 240,
                                   'future_worker_qualification_required': True}
    result['claim_limits'] += ' The old 0/4 timeout trial remains failed and the separate latency-only draft stays inactive. This is a new joint scope/input/prompt and timeout candidate, not a latency-only or controlled causal comparison. Known controls do not exercise nonempty assignments/history; separate offline tests cover those integration rules.'
    result['complete_gold_sources'] = {'cases': str(old_path.relative_to(ROOT)),
                                       'field_expectations': str(annotations_path.relative_to(ROOT)),
                                       'hashes': 'Both exact sources are bound by evidence_sha256; full gold/provenance remain there unchanged.'}
    result['cases'] = [compact_case(case) for case in result['cases']]
    result['field_expectations'] = [compact_expectation(row) for row in result['field_expectations']]
    return result
...
def validate_dispatch_plan(plan):
    ...
    for sequence, job in enumerate(plan['calls']):
        ...
        if data != audit.build_input(originals, job['scope'], assignments=[{} for _ in originals], history=[]):
        ...
        if data.get('existingQuestionCoverage') != []:
            raise ValueError('Known controls must have empty history.')
        for item in data['items'].values():
            if set(item) != {'prompt', 'choices', 'feedback', 'assignment'} or item['assignment'] != {}:
...
def check_integrity(plan):
    validate_dispatch_plan(plan)
    for path, expected in plan['runtime_source_sha256'].items():
        if sha(Path(path)) != expected:
            raise ValueError('Pinned runtime source changed.')
    for path, expected in plan['evidence_sha256'].items():
        if sha(ROOT / path) != expected:
            raise ValueError('Pinned gold or source evidence changed.')
    for name, expected in plan['harness_source_sha256'].items():
        if sha(HERE / name) != expected:
            raise ValueError('Pinned harness changed.')
    for name, expected in plan['prior_trial_sha256'].items():
        if sha(PREVIOUS / name) != expected:
            raise ValueError('Prior frozen evidence changed.')


def finish_capture(capture, path):
    valid = [row for row in capture['calls'] if row.get('assessment', {}).get('structural_valid')]
    failures = [row for row in capture['calls'] if row.get('failure_reason')]
    capture['status'] = ('stopped_global_failure' if capture.get('stop_reason') else
                         'complete_with_failed_batches' if failures else 'complete_pending_independent_audit')
...
def run_calls(client, plan, capture, path, *, sensitive_values=(), monotonic=time.monotonic):
    ...
    initial_plan_digest = digest(plan)
    for job in plan['calls']:
        try:
            if digest(plan) != initial_plan_digest or len(capture['calls']) >= MAX_CALLS:
                raise ValueError('Plan integrity or call ceiling failed.')
            check_integrity(plan)
        except Exception as error:
            capture['stop_reason'] = 'source_or_budget_integrity_failure'
            capture['global_failure'] = safe_error_details(error, sensitive_values=sensitive_values)
            break
        ...
        try:
            ...
            row['failure_reason'] = 'provider_failure'
            code = row['provider_error']['Error']['Code']
            if code in {'ExpiredToken', 'ExpiredTokenException', 'UnrecognizedClientException',
                        'InvalidSignatureException', 'IncompleteSignature', 'AccessDeniedException'} or type(error).__name__ in {
                            'NoCredentialsError', 'PartialCredentialsError', 'CredentialRetrievalError', 'InvalidConfigError', 'ParamValidationError', 'NoRegionError'}:
                capture['stop_reason'] = 'credentials_or_setup_failure'
            elif type(error).__name__ in {'ProviderCallBudgetExceededError', 'ProviderDeadlineExceededError'}:
                capture['stop_reason'] = 'budget_integrity_failure'
        else:
            ...
            try:
                ...
                row['failure_reason'] = 'invalid_provider_response'
            if not row.get('failure_reason'):
                if row['stop_reason'] != 'end_turn':
                    row['failure_reason'] = 'non_end_turn'
                elif elapsed > 100:
                    row['failure_reason'] = 'provider_elapsed_limit'
                else:
                    try:
                        ...
                        row['failure_reason'] = 'content_integrity_failure'
                        capture['stop_reason'] = 'content_integrity_failure'
                    except Exception as error:
                        row['validation_error_type'] = type(error).__name__
                        row['failure_reason'] = 'local_or_binding_failure' if row.get('native_schema_valid') else 'native_schema_failure'
                        if not isinstance(error, (ValueError, TypeError, native.ProviderError)):
                            row['failure_reason'] = 'content_integrity_failure'
                            capture['stop_reason'] = 'content_integrity_failure'
        try:
            if digest(plan) != initial_plan_digest:
                raise ValueError('Plan integrity changed during the call.')
            check_integrity(plan)
        except Exception as error:
            row.pop('assessment', None)
            row['failure_reason'] = 'source_or_budget_integrity_failure'
            capture['stop_reason'] = 'source_or_budget_integrity_failure'
            capture['global_failure'] = safe_error_details(error, sensitive_values=sensitive_values)
        save(path, capture)
        print(json.dumps({key: row.get(key) for key in ('sequence', 'elapsed_seconds', 'native_schema_valid', 'validation_error_type', 'failure_reason')}), flush=True)
        if capture.get('stop_reason'):
            break
    finish_capture(capture, path)
...
def execute(expected_hash):
    ...
    capture = {'plan_sha256': expected_hash, 'status': 'preflight', 'calls': [], 'started_at': datetime.now(timezone.utc).isoformat()}
    sensitive_values = ()
    try:
        ...
        client = boto3.client('bedrock-runtime', region_name='us-east-1', aws_access_key_id=credentials['AccessKeyId'],
                             ...
                             config=Config(connect_timeout=3, read_timeout=100, retries={'total_max_attempts': 1}))
        del credentials
        assert (client.meta.config.connect_timeout, client.meta.config.read_timeout, client.meta.config.retries['total_max_attempts']) == (3, 100, 1)
    except Exception as error:
        capture['stop_reason'] = 'credentials_or_setup_failure'
        capture['global_failure'] = safe_error_details(error, sensitive_values=sensitive_values)
        save(CAPTURE, capture, exclusive=True)
        finish_capture(capture, CAPTURE)
        return
